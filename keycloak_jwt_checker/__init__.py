...
def cli(server_url, client_id, client_secret, realm, username, password) -> None:
    ...
    if access_token:
        ...
        click.echo(pprint.pprint(decode(access_token, client_secret, options={"verify_signature": False})))
    if id_token:
        click.echo("ID TOKEN:")
        click.echo(pprint.pprint(decode(id_token, client_secret, options={"verify_signature": False})))
    if refresh_token:
        click.echo("REFRESH TOKEN:")
        click.echo(pprint.pprint(decode(refresh_token, client_secret, options={"verify_signature": False})))
